...
const Signup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Truncate password to 72 UTF-8 bytes to avoid bcrypt limit on server
    ...
    const safePassword = truncateToMaxBytes(formData.password, 72);
    if (safePassword.length < formData.password.length) {
      setWarning('Password was truncated to 72 bytes for compatibility.');
    }

    setLoading(true);
    try {
      await register(formData.name, formData.email, safePassword);
